...
*
* Changes made:
* 	- LED 13 is on pin PB1
*	- Disable TX / RX functionality
* 	- Add HWB pin sample functionality
*/
...
#define _CDC_H_
	...
	/* Macros: */
		...
		#define LED_SETUP()		DDRB |= (1<<1);
		#define L_LED_OFF()		PORTB &= ~(1<<1)
		#define L_LED_ON()		PORTB |= (1<<1)
		#define L_LED_TOGGLE()	PORTB ^= (1<<1)

 		#define HWB_SETUP()		{DDRE &= ~(1<<2); PORTE |= (1<<2);}	// configure HWB pin (=PE2) as input with pull-up
 		#define HWB_STATUS() 	(PINE & (1<<2))
